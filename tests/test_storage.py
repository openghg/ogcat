...
def test_primary_storage_planner_builds_directory_storage_plan(tmp_path: Path) -> None:
    """Primary planning preserves directory writer semantics in the storage plan."""
    catalog_root = tmp_path / "catalog"
    result = storage_planning.plan_primary_storage(
        storage_planning.PrimaryStoragePlanningContext(
            catalog_root=catalog_root,
    ...
    assert plan.ogcat_owned
    assert plan.adapter == "local"
    assert plan.storage_relative_path == "ab/abcdef1234567890.zarr"
    assert plan.resolved_directory == "ab"
    assert plan.resolved_filename == "abcdef1234567890.zarr"
